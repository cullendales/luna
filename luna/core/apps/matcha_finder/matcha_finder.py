            ...
            if word == "matcha":
                ...
                if i < len(tagged_reviews) - 2:
                    ...
                    if next_word in linking_verbs:
                        if i + 2 < len(tagged_reviews) and tagged_reviews[i + 2][1] in [
                            "JJ",
                            "JJR",
                            "JJS",
                        ]:
                            self.keywords.append(tagged_reviews[i + 2][0])
                        elif i + 3 < len(tagged_reviews) and tagged_reviews[i + 3][
                            1
                        ] in ["JJ", "JJR", "JJS"]:
                    ...
                    # check for same pattern above but after 'latte'
                        ...
                        if third_word in linking_verbs:
                            if i + 3 < len(tagged_reviews) and tagged_reviews[i + 3][
                                1
                            ] in ["JJ", "JJR", "JJS"]:
                                self.keywords.append(tagged_reviews[i + 3][0])
                            elif i + 4 < len(tagged_reviews) and tagged_reviews[i + 4][
                                1
                            ] in ["JJ", "JJR", "JJS"]:
...
class MatchaFinder:
    ...
    def create_table(self):
        self.cursor.execute(
            """CREATE TABLE IF NOT EXISTS matcha_lattes(
            ID TEXT PRIMARY KEY,
            NAME TEXT,
            ADDRESS TEXT,
            LON FLOAT,
    ...
    def get_reviews(self, place_id):
        ...
        params = {"key": self.api_key, "place_id": place_id, "fields": "reviews"}
        response = requests.get(url, params=params)
        response.raise_for_status()
        data = response.json()
        ...
        while len(cafes) < max_cafes:
            ...
            if data.get("status") != "OK":
                ...
                if data.get("status") == "ZERO_RESULTS":
                    print(
                        "There are unfortunately no cafes serving a matcha latte in your area"
                    )
                break
    ...
    def add_cafe(self, cafe, city):
        ...
        self.cursor.execute(
            """INSERT OR IGNORE INTO matcha_lattes (ID, NAME, ADDRESS, LON, LAT, RATING, KEYWORDS, CITY)
                            VALUES (?,?,?,?,?,?,?,?)""",
            (
                cafe.place_id,
                cafe.name,
    ...
    def load_cached_cafes(self, city):
        self.cursor.execute(
            "SELECT * FROM matcha_lattes WHERE CITY = ?", (city.lower(),)
        )
...
def find_matcha(cheetah):
    ...
    if len(cafes) > 5:
        for cafe in cafes[:5]:
            ...
            if cafe.keywords:
                respond(
                    f"People are saying: {', '.join(cafe.keywords)} about their matcha lattes"
                )
    else:
        for cafe in cafes:
            respond(f"{cafe.name} which has a {cafe.rating} star rating")
    ...
    while answer != "quit":
        ...
        respond(
            "Respond one to filter by adjectives in reviews describing a cafe's Matcha Latte"
        )
        ...
        if answer == "one":
            respond("Please say adjectives to filter cafes by")
            response = get_command(cheetah)
            user_keywords = response.split()
            print()
            cafe_list = []
            ...
            if not cafe_list:
                ...
                if len(cafes) > 5:
                    for cafe in cafes[:5]:
                        if cafe.name in cafe_list and cafe.rating >= float(
                            rating_filter
                        ):
                            respond(
                                f"{cafe.name} which has a {cafe.rating} star rating"
                            )
                            if cafe.keywords:
                                respond(
                                    f"People are saying: {', '.join(cafe.keywords)} about their matcha lattes"
        ...
        elif answer == "two" or answer == "to":
            respond(
                "Say a number from 1 - 5 to only see cafes with that rating or higher"
            )
            ...
            respond(
                "The answer you gave was not an option. Please say one of the options below:"
            )
